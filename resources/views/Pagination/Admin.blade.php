<div class="dropdown relative left-0 z-50">
    ...
    <div class="dropdown-box mt-10 absolute w-20 top-0 left-0 z-20">
        <div class="dropdown-box__content box p-2">
            ...
            <form action="{{route($route.'.indexpaginate')}}" method="post">
                @csrf
                <input type="radio" id="name" value="10" name="pagez" hidden checked>
                <input type="submit" name="submit" class="block p-2 transition duration-300 ease-in-out bg-white hover:bg-gray-200 rounded-md w-full" value="10" >
            </form>
            @endif
                @if($paginator->perPage() != 25)
                <form action="{{route($route.'.indexpaginate')}}" method="post">
                @csrf
                <input type="radio" id="name" value="25" name="pagez" hidden checked>
                <input type="submit" name="submit" class="block p-2 transition duration-300 ease-in-out bg-white hover:bg-gray-200 rounded-md w-full" value="25" >
            </form>
                @endif
                @if($paginator->perPage() != 50)
                <form action="{{route($route.'.indexpaginate')}}" method="post">
                @csrf
                <input type="radio" id="name" value="50" name="pagez" hidden checked>
                <input type="submit" name="submit" class="block p-2 transition duration-300 ease-in-out bg-white hover:bg-gray-200 rounded-md w-full" value="50" >
            </form>
                @endif
        </div>
    </div>
