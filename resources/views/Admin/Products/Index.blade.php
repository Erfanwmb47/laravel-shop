...
@section('content')
    ...
    <div class="grid grid-cols-12 gap-6 mt-5">
        <div class="intro-y col-span-12 flex flex-wrap sm:flex-no-wrap items-center mt-2">
            @can('create-product')
                <a href="{{route('products.create')}}" class="button text-white bg-theme-1 shadow-md ml-2">افزودن محصول جدید</a>
            @endcan
                ...
                <form action="" method="post" enctype="multipart/form-data">
                    ...
                    <table class="table table-report sm:mt-2">
                        ...
                        <tr>
                            ...
                            <th class="whitespace-no-wrap text-center">تعداد موجودی</th>
                            <th class="whitespace-no-wrap text-right">نام محصول
                                @if((request()->query('chevron') == 'descending' && request()->query('order_by') == 'name'))
                                    <a href="{{request()->fullUrlWithQuery(['chevron' => 'ascending','order_by'=> 'name'])}}"> <i data-feather="chevron-down" class="w-4 h-4 float-left"></i></a>
                                @else
                                    <a href="{{request()->fullUrlWithQuery(['chevron' => 'descending','order_by'=> 'name'])}}"> <i data-feather="chevron-up" class="w-4 h-4 float-left"></i></a>
                                @endif
                            </th>
                            ...
                            <tr class="intro-x">
                                <td class="w-40">
                                    <div class="flex justify-center items-center">
                                        @can('edit-product')
                                            <a href="{{route('products.edit',$product->id)}}" class="flex items-center ml-3" > <i data-feather="check-square" class="w-4 h-4 ml-1 "></i> </a>
                                        @endcan
                                        @can('delete-product')
                                            <a data-toggle="modal" id="submit" data-target="#UserDelete{{$product->id}}" class="flex items-center text-theme-6" href="javascript:;"> <i data-feather="trash-2" class="w-4 h-4 ml-1"></i> </a>
                                        @endcan
                                    </div>
                                </td>
                                <td class="w-40">
                                ...
                                <td class="text-center">
                                     <a class="button w-24 rounded-full mr-1 mb-2 bg-theme-18 text-theme-9 text-xs"> {{$product->quantity}}</a>
                                </td>
                                <td>
                                    <a href="" class="font-medium whitespace-no-wrap flex-justify-start">{{$product->name}}</a>
                                    <div class="text-gray-600 text-xs whitespace-no-wrap flex-justify-start">{{$product->nickName}}</div>
                                </td>
                                <td class="table-report__action w-56">
                                    <div class="flex w-10 m-auto">
                                        <div class="w-10 h-10 image-fit zoom-in">
                                            <img alt="{{$product->nickName}}" class="tooltip rounded-full" src="{{str_replace('public','/storage',optional($product->gallery)->path)}}" title="آپلود شده در تاریخ {{$product->updated_at}}">
                                        </div>
                                    </div>
                                </td>
                                <td>
        ...
        <div class="mt-5 flex-justify-profile intro-y col-span-12 sm:flex-row sm:flex-no-wrap items-center ">
            {{--                {{$brands->links()}}--}}
            @include('Pagination.Admin', ['paginator' => $products,'route' => 'products'])
        </div>
        <!-- END: Pagination -->
    </div>
