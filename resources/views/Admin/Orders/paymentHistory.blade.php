...
{{--                    <form action="{{route('orders.paymentHistory')}}" method="post" class="flex items-center px-3 py-2 rounded-md  font-medium @if(!side_menu_active(['dashboard/orders/paymentHistory/unpaid','dashboard/orders/paymentHistory/paid'],'bg-theme-22')) bg-theme-22 @endif">--}}
{{--                        @csrf--}}
{{--                        <button class="w-full foc flex">  <i class="w-4 h-4 ml-2 float-right" data-feather="mail"></i> همه پرداختی ها </button>--}}
{{--                    </form>--}}
{{--                    <form method="post" action="{{route('comments.index.approved')}}" class="flex items-center px-3 py-2 mt-2 rounded-md {{side_menu_active('dashboard/orders/paymentHistory/paid','bg-theme-22')}}">--}}
{{--                        @csrf--}}
                        ...
                        @foreach($payments as $payment)
                            <div class="intro-y ">
                                <div class="inbox__item inbox__item--active inline-block sm:block text-gray-700 bg-gray-100 border-b border-gray-200 ">
                                    <div class="flex px-5 py-3 {{$payment->approved == 'allow' ? 'bg-theme-18':'bg-theme-31' }}">
                                        <div class="w-56 flex-none flex items-center mr-10 ">
                                            <input class="input flex-none border border-gray-500" type="checkbox" >

                                            <a href="javascript:;" data-toggle="modal" id="submit" data-target="#CommentsEdit{{$payment->id}}"  class=" w-5 h-5 flex-none mr-4 flex items-center justify-center text-gray-500"> <i class="w-4 h-4" data-feather="edit-2"></i> </a>
                                            <a href="javascript:;" data-toggle="modal" id="submit" data-target="#CommentsDelete{{$payment->id}}"  class=" w-5 h-5 flex-none mr-4 flex items-center justify-center text-gray-500"> <i class="w-4 h-4" data-feather="trash"></i> </a>

{{--                                            <form method="post" action="{{route('comments.update.approved',$payment)}}" title="تایید نظر "class="tooltip w-5 h-5 flex-none mr-2 flex items-center justify-center text-gray-500">--}}
{{--                                                @csrf--}}
{{--                                                @method('PATCH')--}}
...
{{--                                            </form>--}}
                                            <div class="w-6 h-6 flex-none image-fit relative mr-5">
                                                {{--
                                                                                   @dd($payment->commentable->gallery->path)
                                                --}}
{{--                                                <img alt="Midone Tailwind HTML Admin Template" class="rounded-full" src="{{str_replace('public','/storage',$payment->commentable->gallery->path)}}">--}}
                                            </div>
                                            {{--                                TODO tool tip for user name--}}
                                            <div class="inbox__item--sender truncate mr-3 text-right w-20">{{$payment->order->id}}</div>
                                        </div>
                                        <div class="w-64 sm:w-auto truncate rtl"> <span class="inbox__item--highlight mr-1">{{$payment->res_number}}</span> </div>
                                        <div class="inbox__item--time whitespace-no-wrap mr-auto pr-10 rtl">{{$payment->created_at->ago()}}</div>
                                    </div>
                                </div>
                            </div>
                        @endforeach
                ...
                <div class="p-5 flex flex-col sm:flex-row items-center text-center sm:text-left text-gray-600">
                    ...
                    @include('Pagination.Admin', ['paginator' => $payments,'route' => 'payments'])
                </div>
            </div>
            <!-- END: Inbox Content -->
        </div>
    ...
    @foreach($payments as $payment)
        <div class="modal" id="CommentsEdit{{$payment->id}}">
            <div class="modal__content">
                <div class="flex items-center px-5 py-5 sm:py-3 border-b border-gray-200">
                    <h2 class="font-medium text-base mr-auto">ویرایش نظر {{$payment->user->username}}</h2>
                </div>
                <form action="{{route('comments.update',$payment)}}" method="post">
                    @csrf
                    @method('PATCH')
        ...
        <div class="modal" id="CommentsDelete{{$payment->id}}">

            <div class="modal__content">
                <form action="{{route('comments.destroy',$payment)}}" method="post">
                    @csrf
                    @method('DELETE')
                    <div class="p-5 text-center"> <i data-feather="x-circle" class="w-16 h-16 text-theme-6 mx-auto mt-3"></i>
                        <div class="text-3xl mt-5">مطمئنی</div>
                        <div class="text-gray-600 mt-2">حذف نظر کاربر {{$payment->user->username}}</div>
                    </div>
