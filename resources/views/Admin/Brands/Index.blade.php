        ...
        <div class="col-span-12 lg:col-span-9 xxl:col-span-10">
            ...
            <div class="intro-y flex flex-col-reverse sm:flex-row items-center" id="ggg">
                <div class="w-full sm:w-auto relative ml-auto mt-3 sm:mt-0">
                    ...
                    <input type="text" class="input w-full sm:w-64 box px-10 text-gray-700 placeholder-theme-13" placeholder="جست و جو میان برند ها">
                ...
                <div class="w-full sm:w-auto flex-reverse ">
                    <a href="{{route('brands.create')}}" class="button text-white bg-theme-1 shadow-md ml-2">ثبت برند جدید</a>
            ...
            <form action="{{route('brands.multiDestroy')}}" method="post" enctype="multipart/form-data">
                @csrf
                @method('DELETE')
                <div class="ml-0 w-full col-span-12 flex-justify-profile">
                    ...
                    @if(\Illuminate\Support\Facades\Auth::user()->role->brands == 13 || \Illuminate\Support\Facades\Auth::user()->role->brands == 15)
                    <button type="submit" title="حذف موارد انتخاب شده " class="tooltip button bg-theme-6 text-white hidden mr-auto ml-2 mt-3  align-center" id="deleteAll"  name="deleteAll" ><i data-feather="trash-2" class=" w-5 m-auto items-center float-left mr-2"></i>حذف موارد </button>
                    @endif
                </div>
                ...
                <div class="intro-y col-span-6 sm:col-span-4 md:col-span-3 xxl:col-span-2">
                    <div class="file box rounded-md px-5 pt-8 pb-5 px-3 sm:px-5 relative zoom-in">
                        <div class="absolute left-0 top-0 mt-3 ml-3">
                            <input class="input border border-gray-500 marked" onclick="multidelete(this);" value="{{$brand->name}}" name="{{$brand->id}}" type="checkbox">
                        </div>
                        <a href="" class="  mx-auto my-auto">
                            <div class="file__icon__brands m-auto justify-center items-center">
                                @if($brand->gallery)
                                <img src="{{str_replace('public','/storage',$brand->gallery->path)}}" class="brand_image ">
                                @endif
                            </div>
                        </a>
                        <a href="" class="block font-medium mt-4 text-center truncate">{{$brand->name}}</a>
                        <div class="text-gray-600 text-xs text-center">{{$brand->country->name}}</div>
                       ...
                       @if(\Illuminate\Support\Facades\Auth::user()->role->brands > 12)
                        <div class="absolute top-0 right-0 mr-2 mt-2 dropdown ml-auto">
                            ...
                            <div class="dropdown-box mt-5 absolute w-40 top-0 right-0 z-10">
                                <div class="dropdown-box__content box p-2">
                                    @if(\Illuminate\Support\Facades\Auth::user()->role->brands > 13 )
                                    <a href="javascript:;" data-toggle="modal" data-target="#BrandModal{{$brand->id}}" class="flex items-center block p-2 transition duration-300 ease-in-out bg-white hover:bg-gray-200 rounded-md"> <i data-feather="users" class="w-4 h-4 mr-2"></i> ویرایش </a>
                                    @endif
                                        @if(\Illuminate\Support\Facades\Auth::user()->role->brands == 13 || \Illuminate\Support\Facades\Auth::user()->role->brands == 15 )
                                    <a href="javascript:;" data-toggle="modal" id="submit" data-target="#BrandDelete{{$brand->id}}" class="flex items-center block p-2 transition duration-300 ease-in-out bg-white hover:bg-gray-200 rounded-md"> <i data-feather="trash" class="w-4 h-4 mr-2"></i> حذف </a>
                                    @endif
                                    <a href="javascript:;" data-toggle="modal" id="submit" data-target="#BrandEdit2{{$brand->id}}" class="flex items-center block p-2 transition duration-300 ease-in-out bg-white hover:bg-gray-200 rounded-md"> <i data-feather="trash" class="w-4 h-4 mr-2"></i> ویرایش 2 </a>
                                </div>
                            </div>
                        </div>
                       @endif
...
{{--                                {{$brands->links()}}--}}
                @include('Pagination.Admin', ['paginator' => $brands,'route' => 'brands'])
            </div>
            <!-- END: Pagination -->
        </div>
                                        ...
                                        <div class="p-5 " id="input">
                                            <div class="preview grid-rtl grid-cols-12">
                                                <div class="col-span-6 ml-2">
                                                    ...
                                                    <input type="text"  name="name" class="input w-full border mt-2 align-right" style="direction: rtl" placeholder="" value="{{$brand->name}}">
                                                </div>
                                                <div class="col-span-6">
                                                    <label><i data-feather="edit-2" class="w-4 h-4 float-right ml-2 text-blue-400"></i>تگ </label>
                                                    <input type="text" name="tag" class="input w-full border mt-2 align-right" style="direction: rtl" placeholder="" value="{{$brand->tag}}">
                                                </div>
                                        ...
                                        <div class="intro-y grid grid-cols-12 gap-3 sm:gap-6 mt-5 relative overflow-x-auto scrollbar-hidden h-128" id="showGallery">
                                            @foreach($galleries as $gallery)
                                                <div class="intro-y col-span-6 sm:col-span-4 md:col-span-4 xxl:col-span-2">
                                                    <div class="file box rounded-md px-5 pt-8 pb-5 px-3 sm:px-5 relative zoom-in">
                                                        <div class="absolute left-0 top-0 mt-3 ml-3">
                                                            <input class="input border border-gray-500 marked" @if($brand->gallery->id == $gallery->id)checked @endif id="{{$gallery->id}}" value="{{$gallery->id}}" name="selectedImage" type="radio">
                                                        </div>
                                                        <a href="" class="  mx-auto my-auto">
