    ...
    <div class="grid grid-cols-12 gap-6 mt-5 rtl" >
        ...
        @foreach($paymentGateways as $paymentGateway)
            <div class="intro-y col-span-12 md:col-span-6 lg:col-span-4 rtl">
                <div class="box">
                    <form action="{{route('paymentgateways.edit',$paymentGateway)}}">
                        @csrf
                        <div class="flex items-end px-5 pt-5 ">
                            <div class="w-full flex-reverse-start flex-row lg:flex-start ">
                                <div class="w-16 h-16 image-fit">
                                    <img alt="Midone Tailwind HTML Admin Template" class="rounded-md" src="{{str_replace('public','/storage',optional($paymentGateway->gallery)->path)}}">
                                </div>
                                <div class="lg:mr-4 text-center lg:text-right mt-3 lg:mt-0 mr-2">
                                    <a href="" class="font-medium">{{$paymentGateway->name}}</a>
                                    <div class="text-gray-600 text-xs">{{$paymentGateway->eng_name}}</div>
                                </div>
                            </div>
                        </div>
                        <div class="text-center lg:text-right p-5 rtl">
                            ...
                            <div class="flex items-center justify-center lg:justify-end text-gray-600 mt-5">ضریب وزن :
                                {{$paymentGateway->factor_weight}}
                                <i data-feather="hexagon" class="w-3 h-3 ml-2"></i>
                            </div>
                            <div class="flex items-center justify-center lg:justify-end text-gray-600 mt-1 rtl"> ضریب مسافت : {{$paymentGateway->const_distance}}<i data-feather="hexagon" class="w-3 h-3 ml-2"></i></div>
                        </div>
                        <div class="text-center lg:text-right p-5 border-t border-gray-200">
                            ...
                            <a data-toggle="modal" id="submit" data-target="#BrandDelete{{$paymentGateway->id}}" class="button button--sm text-gray-700 border border-gray-300">حذف </a>
                        </div>
                    </form>
                </div>
            </div>
        ...
        <div class="mt-5 flex-justify-profile intro-y col-span-12 sm:flex -row sm:flex-no-wrap items-center ">
            ...
            @include('Pagination.Admin', ['paginator' => $paymentGateways,'route' => 'paymentgateways'])
        </div>
        <!-- END: Pagination -->
    </div>
    @foreach($paymentGateways as $paymentGateway)
        {{--    DELETE Brand--}}
        @can('delete-brand')
            <div class="modal" id="BrandDelete{{$paymentGateway->id}}">
                <div class="modal__content">
                    <form action="{{route('paymentgateways.destroy',$paymentGateway)}}" method="post" >
                        @csrf
                        @method('DELETE')

                        <div class="p-5 text-center"> <i data-feather="x-circle" class="w-16 h-16 text-theme-6 mx-auto mt-3"></i>
                            <div class="text-3xl mt-5">حذف درگاه  <span style="color: darkred">{{$paymentGateway->name}}</span></div>
                            {{--                        <div class="mt-3 ">--}}
